...
*Maintenance note (2026-08-14): build dependencies (`marked`, `mustache`) updated to fix known high-severity security advisories (a ReDoS in `marked`, an XSS in `mustache`); `render.js` adjusted for the new `marked` API. Deploys are automatic: a GitHub Action renders the site on every push to `main` and publishes it straight through GitHub's official Pages deployment (no `gh-pages` branch, no extra git credentials involved). No tutorial content changed.*
